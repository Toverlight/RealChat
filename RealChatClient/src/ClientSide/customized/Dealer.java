...
public class Dealer {
    ...
    private static final double scrHeight = Toolkit.getDefaultToolkit().getScreenSize().getWidth();
...
//      button.setText(null);                                           //除去按钮的默认名称
    ...
    /**
     * 删除所有add方法加入的组件并显式释放内存
     * @param component 组件或容器
     */
    public static void clearPanel(JComponent component) {
        // 获取容器中的所有组件
        Component[] components = component.getComponents();

        // 遍历所有组件并将其设置为null，帮助垃圾回收
        ...
        // 移除所有组件
        component.removeAll();
        component.revalidate();  // 重新验证布局
        component.repaint();     // 重绘面板
    }
}
